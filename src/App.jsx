...
const getUrl = (title, typeTitle='anime',limit=3, fullFields=false) => {
  const urls = ['/mal'];
  if(!(typesTitle.includes(typeTitle))){
    return '';
  }
  if(typeTitle === 'anime') {
    urls.push('anime');
  }
  const queryFields = fullFields? fields: fields.slice(0,fields.indexOf('studios')+1);
  const query = [`q='${title}'`,`limit=${limit}`, `fields=${queryFields.join(',')}`];
  return [urls.join('/'), query.join('&')].join('?');
};
const load = async(title, callbackSet) => {
  const req = await fetch(getUrl(title,'anime',10));
  const resp = await req.json();
  callbackSet(resp.data.map((anime)=>anime.node));
};
const App = () => {
